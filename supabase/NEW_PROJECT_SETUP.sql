-- ============================================================
-- NAYALYZER — NEW SUPABASE PROJECT SETUP
-- Generated from FULL_MIGRATION.sql plus migrations 007-017.
-- Run once in the SQL Editor of the new Supabase project.
-- This creates the schema, RLS, RPCs, storage bucket, and seed function.
-- It does not copy production rows from the old project automatically.
-- ============================================================
...
-- SOURCE: 011_workspace_settings.sql
...
-- SOURCE: 012_member_feature_permissions.sql
...
-- SOURCE: 013_invite_otp_flow.sql
...
-- SOURCE: 014_hidden_platforms.sql
...
-- SOURCE: 015_workspace_assets_storage.sql
...
-- SOURCE: 016_social_accounts_media_count.sql

ALTER TABLE public.social_accounts
  ADD COLUMN IF NOT EXISTS media_count integer DEFAULT 0;


-- SOURCE: 017_social_accounts_profile_fields.sql
...
-- BOOTSTRAP DATA
-- ============================================================
-- The schema and seed function are ready above. For a demo workspace:
-- 1) Create the first user in Supabase Dashboard > Authentication > Users.
-- 2) Sign in to the app so the user becomes the authenticated owner.
-- 3) Create a workspace from the app, then run:
--    SELECT public.seed_workspace('<WORKSPACE_UUID>');
--
-- Do not run the seed function more than once for the same workspace;
-- it inserts sample accounts, metrics, contents, competitors, hypotheses,
-- and reports.
--
-- This file cannot contain the old project's real production rows because
-- those rows are not stored in the repository. Export them from the old
-- database first, then import the dump into the new project.
